...
class Node2:
    ...
    @staticmethod
    def calc_entropy(df):       
        return -sum([x*np.log(x) + (1-x)*np.log(1-x) if x*(1-x) > 0 else 0 for x in df.mean(axis=0)])   
    ...
    def build_tree(self, theta, max_depth=100, min_filters_to_split=2):
        ...
        if (self.depth < max_depth) and (self.num_filters >= min_filters_to_split):
            ...
            self.split_elem = np.argmax(all_info_gains)
    ...
    def traverse(self, subset, weights):
        ...
        for i in range(self.theta.shape[0]):
            dist = sum(weights*abs(self.theta[i] - subset))
            if dist < temp_min:
                temp_min = dist
                temp_filter = self.theta[i]
